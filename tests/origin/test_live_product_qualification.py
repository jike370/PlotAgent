...
@pytest.mark.skipif(
    not RUN_LIVE,
    reason="set PLOTAGENT_RUN_ORIGIN_LIVE_PRODUCT=1 for the 43-chart native gate",
)
def test_all_43_product_charts_survive_one_fresh_origin_reopen(tmp_path: Path) -> None:
    resolved = tuple(resolve_chart(chart_id) for chart_id in PRODUCT_CHART_IDS)
    plan = compile_origin_plan(
        resolved,
        build_origin_export_spec(
            resolved,
            export_id="export:live.product-43",
            target_scope="selected_plots",
        ),
    )
    target = tmp_path / "product-43.opju"
