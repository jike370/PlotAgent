...
> 产品代号：PlotAgent  
> 日期：2026-08-05  
> 相关资料：[已确认产品决策基线](./PRODUCT-DECISIONS.md)、[后端与 Agent 架构](./BACKEND-ARCHITECTURE.md)、[领域契约与 Schema 设计](./DOMAIN-CONTRACTS.md)、[科研图形库调研](./chart-library-research.md)、[产品战略](../PRODUCT.md)、[设计种子](../DESIGN.md)
...
- “结构同构”要求列、数据类型、单位和字段语义完全一致，不允许按文件设置映射例外。
...
3. 列、数据类型、单位和字段语义完全一致的数据自动组成候选批次，不同结构拆成独立候选。
4. 用户从图形库选择类型，或在指令中明确指定图形。
5. 系统根据列名、类型、单位和结构预填字段映射。
6. 用户只确认或调整一次，映射应用到整个批次。
7. 明确指令且无歧义时可跳过映射确认。
8. 第一轮不提供第二轮字段映射，也不允许单个文件设置映射例外。
9. 如需统一异构数据，用户先确认结构转换并生成派生数据，再重新组成批次。
10. 系统生成图集，样式统一，坐标按每张图自动缩放；统一坐标范围只能由用户明确开启。
...
- 原始数据和命名版本不自动删除。
- `.plotproj` 导入数据默认复制进项目，不依赖外部路径；导出项目副本时可选择是否包含原始数据。
