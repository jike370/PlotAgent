...
状态：设计基线；首阶段实施范围已收敛为 T1、T2，尚未宣称完成模板迁移

核对环境：OriginPro 2024 SR1 `10.10.178`，`originpro 1.1.15`

模板来源：本机 Origin 安装目录 `D:\origin`

历史图形范围：`build/visual-audit/per-chart-opju.manifest.json` 中的 45 图

重构后正式范围：仅 T1/T2，共 38 图

## 1. 使用原则

本表回答的是“每张图优先从哪个 Origin 官方资产创建”，不是证明现有 renderer 已经采用该模板。
