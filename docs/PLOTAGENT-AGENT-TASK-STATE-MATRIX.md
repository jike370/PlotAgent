...
>
> 边界：本文只证明任务生命周期和副作用语义，不证明 34 个图类的字段合同、编辑参数、双后端映射、导出或 UI 控件完整性。产品能力覆盖由《PlotAgent 产品测试覆盖审计》单独约束，二者必须同时通过。
...
| SM-65 | 数据更新完整撤销与重做 | 已有图版本 N → `update_data` 生成 N+1 → 撤销生成 N+2 → 重做生成 N+3 | 历史项保存来源版本与结果版本；Core 以不可变 PlotDocument/backend 原生快照恢复，保留完整 `data` 与 `bindings`，禁止只恢复部分字段或猜测 renderer 默认值 | `tests/engine/test_engine_runtime.py::test_runtime_restores_an_exact_snapshot_then_appends_new_edits_to_that_state`、`src/renderer/src/App.test.tsx::undoes and redoes an Agent data update with exact version snapshots` |
...
| SM-69 | 轴范围编辑完整撤销 | 自动范围 → 固定上下限 → 撤销/重做 | 编辑仍使用显式 `bounds_mode=automatic|fixed` 表达范围语义；历史恢复以来源版本原生快照回滚/重做，Origin 与 Matplotlib 不再依赖前端猜测；同轮标题/轴/图例编辑形成一个完整历史项 | `tests/engine/test_visual_t1.py::test_axis_bounds_can_transition_between_fixed_and_automatic`、`src/renderer/src/App.test.tsx::records a compound Agent edit with implicit renderer defaults in exact snapshot history` |
