...
- **实现选择（2026-08-06，Origin P1 扩展）：** registry 扩为 52 个稳定 ID，新增 21 图继续复用现有 adapter family，但哑铃/棒棒糖/蜂群/浮动柱/人口金字塔/矩阵边缘分布/多种双 Y/Y 偏移/火山图等由 chart-specific fixed resolver 生成确定性几何。X09/X35 区间柱使用 Origin 原生 XYY Floating Column：第一 Y 是对象级透明的起始边界，第二 Y 是可见终点，按区间宽度设置组内柱间距；因此 OPJU 保持数据联动、直角矩形端部、无零基线细线，也不依赖全局线帽设置。水平发散柱继续使用原生 area polygon。双 Y 使用重叠原生 layer、独立左右 scale/labels，并隐藏重复 X 轴；双 Y 柱通过左右错开的原生 Floating Column 保证跨图层不重叠。X02 使用单一原生 Scatter + Drop Lines，横轴保留标准 bottom 位置，Origin 用户调整 Y 显示范围后棒仍自动连接坐标框底部；Matplotlib 同样以已解析 Y 轴的可见下边界绘制棒。Origin worker 强制 UTF-8 进程环境，跨进程 plan hash 使用 ASCII Unicode escape；Manifest 分块固定为 3000 字符，规避 Origin automation 约 4 KiB 单元格截断。视觉测试先锚定 A/C 级 Origin 示例与同源数据，当前 10 项进入三栏审计，另 11 项不以合成数据冒充验收。
...
- **当前回归证据（2026-08-06）：** Python 常规门禁为 515 passed、53 个真实 Origin marker skipped，Ruff 与 mypy 全通过；Node/Electron 为 16 files、67 tests，lint、两套 TypeScript typecheck 与 production build 全通过；Windows release tools 离线测试通过。图形代码面已扩展至 52 图（每图 3 组离线 fixture）；其中新增 P1 的 10 个同源证据案例已完成 Matplotlib、Origin 构建与 fresh reopen 复核，其余 11 图因缺少同源参考图与数据对而按门禁暂不做视觉验收。
