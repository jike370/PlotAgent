...
5. 完成正式 38 图的独立 Matplotlib renderer 与 Origin 官方模板绑定器。
6. 完成 38 图默认/代表性编辑机械读回和视觉审查页。

## 下一步顺序

1. 产品负责人完成 38 图视觉签名；失败图回到对应 Profile 修复，不改公共架构。
2. 正式 Electron 黑盒验证导入、公共编辑、Agent 计划、批量/部分失败恢复、重启、K25 与三格式导出。
3. 对黑盒失败做最小修正并复测；证据不足保持 UNVERIFIED。
4. 完成性能、安装包、签名和唯一支持 Origin 版本的发布门禁。
...
工程测试全绿只是必要条件。正式完成还要求 38 图人工视觉签名、桌面黑盒证据和发布门禁全部通过。
