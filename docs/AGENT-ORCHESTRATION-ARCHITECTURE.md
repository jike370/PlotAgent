...
Agent 整理方案经用户确认并发布后即可保存 DataPreparationRecipe，不需要等待绘图或导出完成。确认卡默认展示机械步骤、输出表数量/行列结构和风险，不把内部规则表或整段原始数据直接摊给用户；数据样本只在用户主动展开或定位问题时显示。后续绘图、导出或更多同构文件成功可以登记为验证证据，但不会把字段绑定、单位换算、图类、视觉参数或 renderer 动作追加进 Recipe。
...
3. **Agent 辅助整理且已有绘图目标**：Agent 可以基于临时候选继续完成字段绑定和 TaskDraft；最终确认卡展示机械步骤、输出结构、风险、字段绑定、图类和视觉参数，用户一次确认。原始片段和行样本只按需展开。
4. **Agent 辅助整理但尚无绘图目标**：使用独立数据整理确认卡，默认只展示“如何整理 / 预计输出 / 需要核对”；确认前候选不进入正式数据列表、不推进项目版本，确认后才发布规则 SourceDataset。
...
- 只有实际成功的整理 trace 且整理方案经用户确认发布后才能固化；不要求先完成绘图或导出。
