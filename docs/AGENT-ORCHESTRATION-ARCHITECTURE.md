...
- 用户显式选择的数据源和图形对象，作为结构化硬约束；创建任务的 UI 图类选择作为 Agent
  默认建议，最新自然语言明确指定另一闭集图类时可覆盖；
- 字段目录、逻辑类型、单位候选及候选来源；
- 当前图形的 profile、版本和公开语义对象；
- 上一 TaskPlan、失败项和稳定错误（适用时）；
- 允许的图类、数据操作、视觉动作、工具与预算。

数据源、`@图N` 解析出的 plot ID/version 等授权边界由 Core 强制执行。创建任务的 UI 图类
选择不构成权限边界：它在自然语言未明确图类时提供默认值；自然语言明确覆盖时，Core 只
验证 Agent 产出的结构化 profile 仍属于闭集和本次 capability。自然语言要求由 Agent 解释，
并完整展示在确认卡中。Core 不通过第二套自然语言解析器声称验证“模型是否理解了用户”；
语义忠实度由确认卡、冻结评测和黑盒测试证明。
...
`convert_type` 必须严格失败并报告来源行，不能把非法文本静默改为缺失。普通 numeric parse 只接受数值标量或数值文本；日期转日序数必须显式声明 `datetime_format` 与 `datetime_numeric_mode=ordinal_day`。`align_sources_on_x` 只处理“每个来源一个 X/数值系列，且 X 已按相同顺序对齐”的情形；它生成 renderer 可直接绑定的宽表，但绝不代替排序、插值或科学配准。X 不一致时 Agent 必须根据用户目标另行处理或追问。
...
- 已有当前图且用户未显式选择新建时，前端把当前图作为默认编辑目标；`@图N` 覆盖该默认，显式“新建图”则发送空 `selected_plots`；
...
- 用“没有 `@`”暗中表示新建，或在携带 `selected_plots` 时允许 Agent 产生 `create` item；
...
- 不得静默改变其他已确认项；
- 语义变化必须再次确认；
- 只有非 `WORKFLOW_*`、`known_none` 的确定性技术故障允许同参数安全重试，且最多一次；重复失败后必须修订任务或停止。数据转换、字段绑定、形状和对齐错误证明原计划无效，必须交回 Agent 修订或追问；
