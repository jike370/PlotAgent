...
> 状态：视觉能力设计基线，先于样式 Schema 和 renderer 施工。
...
|---|---:|---|
| 创建、字段绑定 | 34/34 | 已开放 |
| 图标题 | 34/34 | 已开放，但只有文本；没有字体、字号和颜色 |
| 轴标题 | 34/34 | 已开放 |
| 轴类型 | 27/34 | 部分开放：linear/log10/datetime/categorical |
| 轴范围 | 29/34 | 部分开放 |
| 轴反转 | 33/34 | 部分开放 |
| 系列颜色 | 28/34 | 部分开放；一个 `color` 同时承担线、点或填充，语义不够精确 |
| 线宽 | 20/34 | 部分开放 |
| 线型 | 13/34 | 部分开放：solid/dash/dot/dash_dot/none |
| 符号形状 | 12/34 | 部分开放 |
| 符号大小 | 11/34 | 部分开放 |
| 图例显示 | 29/34 | 部分开放；profile 尚未开放位置参数 |
| 图类参数 | 5/34 | 仅气泡标尺/色带、相关矩阵三角、等高线层级、Nyquist 等比例、混淆矩阵计数 |
| 文本标注 | 0/34 | 合同存在，但没有正式 profile 开放 |
| PNG/SVG/OPJU 导出 | 34/34 | 已开放 |

当前公共合同没有独立表达：

- 线透明度和连接方式；
- 符号填充色、边缘色、边缘宽度、实心/空心；
- 柱、面积、误差带的填充和边框；
- 字体、字号、字重和文字颜色；
- 刻度、网格和参考线；
- 数据标签；
...
连续参数不应做成离散选项：符号大小和边缘宽度由 profile 给出合法范围，UI 使用数值输入或滑杆。
