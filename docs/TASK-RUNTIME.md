...
WorkflowRun 依次经过 routing、确定性/Recipe/Agent 路径、draft_ready、awaiting_confirmation、executing 和终态。TaskPlan 依次经过 awaiting_confirmation、ready、running 与 succeeded/partially_succeeded/failed/rejected/cancelled。
...
- 数据处理只允许 TaskDraft Schema 中的封闭操作；原始 SourceDataset 不可变。
