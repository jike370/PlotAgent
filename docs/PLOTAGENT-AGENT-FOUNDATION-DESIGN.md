...
## 最终运行时补充决策（2026-08-19）

### AgentActivation 预算不是产品时长预算

- 用户已确认不设置端到端时长预算；任务持续时间继续由真实事件展示，并完整记录延迟、token、模型调用和成本。
- 单次 AgentActivation 仍必须有结构安全边界。复杂任务可能需要检查多个数据源、图类合同和动作参数，6 个模型轮次会在模型通信正常时产生误阻断，因此当前上限为 10。
- task-wide 上限保持 64 turns / 24 model calls，数据披露、工具、Origin、repair、ExecutionGrant 和副作用边界不变；轮数增加不等于扩大权限。
- 耗尽 10 轮仍返回明确失败或追问，确认前不得修改项目；不得把 `budget_exhausted/model_turns` 解释为余额不足、网络失败或代理故障。

### Windows writer lock 的进程身份

- 单独记录 PID 不足以识别项目 writer，因为 Windows 会复用已经退出进程的 PID。
- writer lease 记录 PID 和进程启动时间；重开时二者同时匹配才视为原 writer 仍存活。旧格式锁可用锁文件时间与当前进程创建时间安全迁移判断。
- 只清理能够机械证明为陈旧的锁；无法证明时保持 fail closed，不通过终止无关进程抢占项目。

### 选择状态与异构批量

- 已选图形代表当前图的权威编辑上下文，不应被一个明确要求多个不同图类的批量任务隐式覆盖。
- 异构新建批量从无预选图形的对话状态发起；TaskIntent 为每个 TaskItem 显式保存数据源、图类和字段绑定。
- UI 的批量入口只传递当前数据表和用户明确加入 Agent 上下文的数据表，不能把项目全部历史数据静默注入模型。
- 批量规划信息不足时返回一条可继续的 `needs_input` 对话结果，不转换成笼统 Core 错误，也不产生半计划或项目副作用。
