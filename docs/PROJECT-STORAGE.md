...
Agent 辅助整理在确认前只保存临时预览/计划，不登记正式 SourceDataset。确认后数据整理项先提交并可被后续 TaskItem 引用；下游 renderer 失败不回滚该数据版本。重新整理创建新版本，旧版本只要仍被 PlotDocument/任务引用就保留；撤销表现为停用/移出当前视图，不破坏历史血缘。
