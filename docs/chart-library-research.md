...
> 本文是157项长期研究taxonomy，不是当前产品目录或候选清单。PlotAgent v3公开产品固定为34张单图且不支持组合图；其中出现的K16核密度图、K25多面板组合图、S01 Kaplan–Meier生存曲线、S21森林图仅保留为研究条目，已从图形库、Agent、双后端与导出能力删除，旧项目引用只返回 `CHART_TYPE_REMOVED`。
