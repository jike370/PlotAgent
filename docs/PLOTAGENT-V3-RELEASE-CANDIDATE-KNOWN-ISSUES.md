...
> 冻结日期：2026-08-24
> 历史说明：本文保留该候选的原始台账；其中 32 来源边界已被 2026-08-25 的 64 来源统一合同取代，当前状态以《施工、测试与发布路线》和《产品测试覆盖审计》第 15 节为准。
...
| `ORIGIN-FRAME-01` | Origin 产品默认轴框不能机械覆盖所有模板和所有图层 | 旧候选给每个原生图层强制打开 bottom/left/top/right；全量保留模板又无法满足标准科研笛卡尔图的统一四边框默认。2026-08-26 确认按原生拓扑区分。 | 24 个标准单层笛卡尔 Profile 仅补齐模板缺失边，上、右关闭主/次刻度线和刻度值；矩阵/色图、原生 Trellis、中央轴和重叠双轴共 10 个 Profile 保持官方拓扑。显式轴线编辑优先，不改 Matplotlib。 | 聚焦测试覆盖适用集合、10 个例外、干净上/右边、显式覆盖、结构漂移和 fresh-reopen；24 个受影响 Profile 重新生成 Origin 与视觉证据，10 个例外证明未被共享层写入。 |
...
| `EXPORT-NAME-01` | OPJU、PNG、SVG 保存框使用稳定图编号、图类编号、中文图名和图版本生成可读文件名；Main 统一清理 Windows 非法字符、保留正确扩展名并限制长度 | `5a3c323` 已实现并通过全量发布门禁；打包版正式 UI 默认显示 `图1-K02-线点图-v5.opju` | 未手工改名直接保存成功，OPJU 30,291 B，SHA-256 `51B7BC84BFDEE0342EEA5D9FDFE8B595F7F1E9BAF5384179F3B3994A02B56EFF`；证据见 `build/opju-default-name-5a3c323-20260826/REPORT.md`。 |
...
当前无用户明确暂缓项。`EXPORT-NAME-01` 已在 `5a3c323` 关闭并移入第 2 节。
...
| `ORIGIN-FRAME-01` | Origin 共享 visual pass 读取模板逐层逐侧状态；对标准单层笛卡尔 Profile 只补齐缺失边并清除上/右刻度与刻度值，对 10 个特殊拓扑 Profile 不写默认轴框；显式 `set_axis` 写入继续优先。 | 旧 `50 passed` 仅证明全量模板保留方案；当前选择性产品默认须重新取证。 | 新提交上的适用/例外聚焦测试、24 图 Origin 与视觉证据、公共轴线编辑矩阵。 |
