...
| ID | 内容 | 状态 | 处理 |
| --- | --- | --- | --- |
| `MOTION-01` | 克制的时间线进入、阶段文字切换、导出提示、任务抽屉、模型设置和导出菜单动效；含 `prefers-reduced-motion` 降级 | `cfb8b5c` 已由用户在 `codex/motion-design` 分支验收 | 在上述功能问题集中修复时合入，不单独改变领域状态；合入冲突按当前结构化时间线为准。 |
