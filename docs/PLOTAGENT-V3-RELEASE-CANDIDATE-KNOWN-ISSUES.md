...
| `ORIGIN-FRAME-01` | 共享后处理层错误地把所有 Origin 图统一成四侧全边框 | 旧候选为关闭一次视觉差异，在所有官方模板之后强制打开 bottom/left/top/right，抹平了模板家族原本不同的轴框设计。产品负责人已撤销该默认规则，接受候选完整重跑。 | 删除共享强制全边框；在任何显式编辑前机械读取每层四侧模板状态，应用用户动作后 fresh-reopen：未编辑侧必须保持模板值，显式编辑侧必须等于用户值。不得逐图补丁，也不得影响 Matplotlib 默认态。 | 聚焦测试覆盖模板状态保留、多图层、显式覆盖、结构漂移和读回失败；真实代表图与新候选 34 图 Origin fresh-reopen 保持各自官方模板默认轴框，公共轴线编辑仍通过。 |
...
| `ORIGIN-FRAME-01` | Origin 共享 visual pass 不再写入默认轴框；它在编辑前记录官方模板逐层逐侧状态，只对显式 `set_axis` 写入目标侧，fresh reopen 同时核对模板保留与用户覆盖。 | 新口径聚焦 `50 passed`；旧“四侧统一”证据只作为被撤销方案的历史记录。 | 新提交上的代表图 pilot、34 图 Origin、视觉签名与公共编辑矩阵。 |
