...
| `ORIGIN-FRAME-01` | Origin 产品默认轴框不能机械覆盖所有模板和所有图层 | 旧候选给每个原生图层强制打开 bottom/left/top/right；全量保留模板又无法满足标准科研笛卡尔图的统一四边框默认。2026-08-26 确认按原生拓扑区分。 | 24 个标准单层笛卡尔 Profile 仅补齐模板缺失边；矩阵/色图、原生 Trellis、中央轴和重叠双轴共 10 个 Profile 保持官方拓扑。显式轴线编辑优先，不改 Matplotlib。 | 聚焦测试覆盖适用集合、10 个例外、显式覆盖、结构漂移和 fresh-reopen；24 个受影响 Profile 重新生成 Origin 与视觉证据，10 个例外证明未被共享层写入。 |
...
| `ORIGIN-FRAME-01` | Origin 共享 visual pass 读取模板逐层逐侧状态；对标准单层笛卡尔 Profile 只补齐缺失边，对 10 个特殊拓扑 Profile 不写默认轴框；显式 `set_axis` 写入继续优先。 | 旧 `50 passed` 仅证明全量模板保留方案；当前选择性产品默认须重新取证。 | 新提交上的适用/例外聚焦测试、24 图 Origin 与视觉证据、公共轴线编辑矩阵。 |
