...
> 状态：候选 Profile 保留 38 个；Origin/OPJU 正式范围为 35 图，核密度图、Kaplan–Meier 生存曲线、森林图稳定拒绝；Agent Native 绘图引擎已替换旧编译链，人工视觉与正式黑盒尚未签名。
...
- **PD-AC19 视觉审查后置且一次性交付。** 迁移中不做探索性或分批视觉审查；先完成35图双后端、动态数据、共同编辑、OPJU/fresh-reopen和逐图机械修改读回，再统一生成35图审查页交由用户逐图判断。机械完成不自动产生视觉 PASS，用户签名前一律保持 `UNVERIFIED`；人工 Origin 实际编辑另按模板家族抽代表图。
...
- **PD-AD08 完成定义。** 工程测试、机械读回、人工视觉和正式桌面黑盒分开记账；当前35图已交付统一视觉审查但尚未取得用户签名，不能宣称发布完成。三个拒绝项单列，不计作通过或未验证。
