...
- Recipe 只能从实际成功的整理 trace 抽取；用户确认整理动作、输出结构和风险并发布结果后即可保存，不等待绘图或导出。初版严格并默认进入个人库，项目固定不可变版本；泛化/修改生成新版本。执行失败回退 Agent，不能让 Recipe 自我放宽或静默改写。
...
- 通用解析/唯一 Recipe 的严格结果自动发布但可查看和重新整理；同级候选先由用户选择。Agent 辅助整理在已有绘图目标时与字段绑定/图类集中确认，仅上传时单独确认；确认卡默认展示机械步骤、输出结构和风险，数据样本按需展开。候选确认前不进入正式数据列表、不推进项目版本；批量按文件部分成功，重新整理生成新 SourceDataset 版本，已有图不静默重绑定。
