# fig-agent 产品决策（内部工程：PlotAgent v3）

> 状态：当前权威决策。本文只描述现行产品，不保留已退役架构、图类或兼容方案。

## A. 产品定位

- 用户可见产品名统一为 `fig-agent`，官网使用 `fig-agent.cn`。内部 Python 包、RPC、存储标识和历史工程名称继续使用 `plotagent` 或 `PlotAgent v3`，不因品牌调整迁移已有工程标识。
- fig-agent 是面向科研绘图目标的 Windows 本地 Agent 产品，同时提供可由其他 Agent 调用的本地绘图引擎。
- 核心价值是让用户直接导入原始表格数据，用自然语言完成单图、批量绘图和持续改图，并把受支持的改动写入可编辑的 Origin 原生图。
...
- 支持 CSV/TSV/TXT/DAT/XLS/XLSX/XLSM 以及多工作表 Excel 的只读导入；仪器前导信息、数据块与尾部元数据分离保存。
- 用户不需要先把数据整理成 fig-agent 的固定模板。导入器机械识别表头、字段类型、单位候选、数据区域和来源；只有存在多个同等合理解释时才提出完成导入所需的最小问题。
...
- PNG/SVG 不依赖 Origin。OPJU 由本机 Origin 创建；自然语言和界面修改进入同一图形版本，并在再次导出时写入 Origin 原生对象，不宣称会反向导入或直接修改任意既有 OPJU。
- 当前程序只接受文件版本 `10.1.0` 的 Origin 2024。OriginPro 2024 SR1（`10.1.0.178`）已经完成完整验证；OriginPro 2024 SR0 和标准版 Origin 2024 不被版本校验拦截，但尚未完成同等范围验证；其他版本稳定拒绝。
- 用户可以手动选择 Origin 主程序。选择结果持久化并同时用于版本检测与实际导出；检测到不兼容版本时显示真实版本，不把它误报为未安装。
- Windows 安装包包含运行所需的 Python 环境和 Origin 连接组件，不要求用户另行安装 Python、配置环境变量或安装 `originpro` 包。
- 导出目标路径只由桌面保存对话框授权，不属于 Agent 权限。
- 凭据只存 Windows Credential Manager；renderer 保持 sandbox/context isolation，无 Node integration。
- 项目、图形版本和导出结果保存在本机。使用自然语言任务时，用户指令、字段元数据和受控样本会发送到用户选择的模型服务；产品不能把“本地项目”表述为“所有数据永不离开本机”。
