...
所有生产页面与批量页面使用同一应用壳和设计系统，不保留新旧壳混用。设计 demo 可以先行验证结构，真实页面必须使用稳定领域对象和真实 Core/Agent 结果；未实现能力不显示假按钮、占位页或 mock 结果。
...
**Display Font:** 拉丁字符使用 `Segoe UI Variable Text` / `Segoe UI`；CJK 字符与全角标点通过 `unicode-range` 优先使用 `Microsoft YaHei UI` / `Microsoft YaHei`；最后回退 `system-ui`, `sans-serif`。
**Body Font:** 与 Display Font 使用同一按字符回退策略，避免中文括号、引号等全角标点落入缺字字体。
...
- 应用无需账号、邀请码或联网即可直接进入主窗口空状态；模型服务只在首次需要 Agent 时轻量配置，不使用多页向导或账号表单。
- 空状态提供三个清晰层级的入口：主按钮“示例”、次按钮“导入”、文字入口“打开已有 `.plotproj`”。
...
- 缺少数据、图形类型或字段时，自然语言仍可提交；Agent 在时间线中说明缺失条件并提供对应入口。这些条件只阻止执行；同一模型请求期间输入串行化。
- 用户消息必须作为右侧气泡保留；Agent 的理解、追问、确认、执行状态、错误和完成沿同一左侧时间线推进。独立表单和任务卡不得脱离产生它们的消息。
- 当前图形是时间线中的已完成对象；用户继续发消息后，新用户气泡、Agent 阶段、确认卡和导出结果必须追加在该图形之后，不能因“当前图”采用固定投影而倒序插到图形上方。
- 时间线以持久化事件序列为唯一顺序来源。计划卡按 `plan_id` 保留同一位置并原位更新，新的图形版本和导出记录各自形成后续对象；`currentPlot` 只是操作投影，不参与历史排序。
- 每张图在项目内获得稳定的 `@图N`，编号单调递增且删除后不复用。Composer 输入 `@` 显示编号、图类和最新版本；用户可显式提及一个或多个编号，未写编号时不把“当前图”“上一张图”等模糊指代自动解析成目标。
- 同一项目同时只显示并执行一个前台 Agent 任务；这不限制其他项目独立存在或继续任务。需要 Origin 的底层步骤由受控资源队列串行，不把该约束伪装成全局项目锁。
- 导入结构确认与选图后的字段语义映射分开展示；图形需要外部分析结果时，在详情页和执行前明确标记“需要预计算字段”，不隐藏图形。
- 输入框附近常驻显示作用对象与范围；图形、系列、坐标轴和图例选中后显示目标标签。当前不提供独立文本标注或自由面板对象。
- 字段映射和变更使用嵌入式确认卡，直接显示角色→字段、目标对象/版本、renderer目标、警告和具体变更，不以泛化动作标题代替内容。
- 同一任务执行期间只保留一条动态状态消息，随真实阶段更新；完成后折叠为执行摘要和结果卡，不把每个内部步骤堆成消息。
- 运行中的状态使用单个呼吸圆点与当前真实阶段文本，不显示伪造百分比或剩余时间。减少动态效果时取消呼吸和位移，但状态文本与可访问播报保持完整。
- 错误与部分成功留在原任务消息中，并提供停止、局部重试或继续入口；页面顶栏通知只作辅助。
- 导出成功同时产生短时醒目提示和可留存的导出结果条；两处均显示文件名，结果条保留文件大小/哈希，并提供“打开文件”“打开所在文件夹”。Renderer 只持有随机资源 ID，真实路径不进入页面状态。
- 可精确构造逆动作的编辑形成新版本并提供“撤销本轮”；`Ctrl+Z`/`Ctrl+Shift+Z`执行会话内撤销/重做，聚焦编辑可比较当前与直接父版本。恢复不删除历史，也不删除已经写出的用户导出文件。
- 只有需要精确控制时进入全窗口聚焦编辑；检查器按需打开，不形成常驻右栏。
- 当前图的“编辑图形”入口直接打开按需检查器；首阶段只显示常规、系列、坐标轴、图例四类已能映射到 Matplotlib 与 Origin 原生对象的参数，不展示伪批次或尚未接线的高级面板。

### 项目资源与批量审阅

- 项目资源库后续从项目标题打开为覆盖层/抽屉，支持资源、版本、血缘、归档和删除依赖查看；当前不显示伪入口。
- 当前批量结果使用真实缩略图网格，支持多选、状态筛选、异常标记和失败项局部重试；列表、轮播、自由排序和叠加比较不在当前范围。
...
- **Don't** 暴露通用分析/拟合入口，或用模糊“自动计算”掩盖预计算字段要求；当前数据处理仅限 Agent 可调用的封闭工具。
