...
当前仓库包含一个可运行的 Windows 桌面应用，以及 W8/M6 的最小 Beta 云控制面实现。界面覆盖项目与对话、结构化数据对象、批量图集、52 项纯数值数据图形目录、聚焦编辑、固定布局组合图、任务状态与 Origin 不可用降级。真实 Agent provider、Python 绘图工作进程和受控 Origin 自动化均已有本地实现；控制面尚未部署生产云，M7 邀请制 Beta qualification 也尚未执行。
...
v1 基线面向小规模邀请制 Beta：保留原 31 图，并增加 21 项 Origin P1 图形（双 Y 轴网格图除外），形成 52 图代码面；支持同构批量/固定组合和 PNG/SVG/O1 OPJU。数据能力仍收敛为确定性 Excel/TXT/CSV 导入、一次字段语义映射、受控 PreparationSpec 与九类固定 PlotCalculation。需要回归、相关、KM、剂量反应等分析的图形只接受预计算字段；通用数据变换、AnalysisSpec/FitSpec 与科学分析/拟合引擎均为后续阶段。新增 P1 的视觉准入坚持“Origin 示例图与同源数据成对”门槛：当前 10 项已完成同源三栏审计，缺少数据的 11 项只保留代码与离线契约测试，不冒充视觉验收。
