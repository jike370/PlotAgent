...
class DurableAgentCoreHost:
    ...
    def _system_prompt(context: AgentContextSnapshot) -> str:
        ...
        return (
            ...
            "After align_sources_on_x, preserve every aligned numeric output as a real plotted "
            "series. If the target profile exposes repeatable series_N roles, bind every aligned "
            "output to those roles in source order. If the target profile instead exposes x, y, "
            "and optional group (including K01, K02, and K03), immediately follow alignment with "
            "reshape_wide_to_long: preserve the aligned X as the id field, pass every aligned "
            "series output as a value field, bind output_value to y, and bind the categorical "
            "output_name to group. Never bind an aligned numeric series output directly to group. "
    ...
    def _environment(self, runtime: _ActivationRuntime) -> dict[str, object]:
        ...
        if runtime.activation.task_state == "repairing":
            system_prompt += (
                ...
                "decision and a declare_unit operation that cites that decision. "
                "For WORKFLOW_ALIGNED_SERIES_ROLE_INVALID, keep align_sources_on_x and add "
                "reshape_wide_to_long over every aligned series output when the profile uses "
                "x/y/group, then bind the categorical name output to group and numeric value "
                "output to y. Never repair it by assigning a numeric aligned column to group. "
                "If a missing "
