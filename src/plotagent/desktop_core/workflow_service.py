...
class DesktopWorkflowService:
    ...
    def _system_prompt(self, context: WorkflowContext) -> str:
        ...
        return (
            ...
            "数据处理必须落成 TaskDraftItem.data_operations，而不是只在 summary 或 detail 中描述。"
            "用户说筛选、只保留、排除或大于等于等条件时，使用 filter_rows；用户说排序、"
            "从高到低或从低到高时，使用 sort_rows；用户说单位换算、转成长宽表、合并、去重、"
            "派生列或类型转换时，使用对应的封闭 data operation。每个 operation 必须引用当前"
            "上下文中的 opaque field_alias/source_alias，并按用户原文顺序组成 operation chain。"
            "例如用户要求‘筛选温度大于等于30，再按Response_mV从高到低排序’，TaskDraftItem"
            "必须同时包含 filter_rows(predicate field_alias=温度, operator=greater_or_equal,"
            "value=30) 和 sort_rows(key field_alias=Response_mV, direction=descending)，"
