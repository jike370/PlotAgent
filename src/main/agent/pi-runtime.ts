...
export class PiAgentRuntime {
  ...
  async prepareData(params: JsonValue): Promise<JsonValue> {
    const input = record(params, 'Pi data preparation request')
    ...
    const preparationRunId = typeof input.preparation_run_id === 'string'
      ? input.preparation_run_id : ''
    const sourcePath = typeof input.source_path === 'string' ? input.source_path : ''
    if (!projectId || !clientRunId || !preparationRunId || !sourcePath) {
      throw new PiRuntimeError('PI_RUNTIME_PROTOCOL_INVALID', 'Missing preparation identity.')
    }
    ...
    let outputTokens = 0
    try {
      this.emit(clientRunId, projectId, 'preparing_context', '正在读取原始文件的受限结构证据…')
      const evidence = await this.core.request('data_preparation.sources.inspect', {
      ...
      const tools: AgentTool<TSchema, JsonValue>[] = [
        {
          name: 'submit_parser_options',
          label: '提交数据整理参数',
          description: '只提交由证据支持的解析参数；不得选择绘图字段、换算单位或生成绘图动作。',
          ...
          execute: async (_toolCallId, args) => {
            toolCalls += 1
            const payload = record(args as JsonValue, 'Preparation proposal')
            const options = record(payload.options, 'Preparation parser options')
            if (Object.keys(options).length === 0) {
              return {
                content: [{ type: 'text', text: 'At least one parser option is required.' }],
                details: { code: 'PREPARATION_OPTIONS_EMPTY' },
                terminate: false,
              }
            }
            finalOutcome = {
              ...
              model_turn_count: completedTurns + 1,
              tool_call_count: toolCalls,
            }
            return {
              content: [{ type: 'text', text: 'Preparation parser options proposed.' }],
        ...
        {
          name: 'report_preparation_unresolved',
          label: '说明无法安全整理',
          description: '只有现有证据不足以安全选择解析参数时使用；不得猜测。',
          parameters: objectSchema({
          ...
          execute: async (_toolCallId, args) => {
            toolCalls += 1
            const payload = record(args as JsonValue, 'Preparation unresolved result')
            finalOutcome = {
              outcome: 'unresolved',
              preparation_run_id: preparationRunId,
              reason: payload.reason ?? '现有证据不足。',
              model_turn_count: completedTurns + 1,
              tool_call_count: toolCalls,
            }
            return {
              content: [{ type: 'text', text: 'Preparation remains unresolved.' }],
              details: finalOutcome,
              terminate: true,
            }
          },
      ...
      agent = new Agent({
        ...
        shouldStopAfterTurn: () => {
          completedTurns += 1
          return finalOutcome !== undefined || completedTurns >= 2
        },
        sessionId: preparationRunId,
      })
      agent.subscribe((event) => {
        ...
        if (next !== undefined) this.emit(clientRunId, projectId, next.stage, '正在判断安全的解析参数…')
      })
      this.active = { runId: clientRunId, generation, agent }
      await Promise.race([
        agent.prompt(JSON.stringify({
          raw_source_evidence: evidence,
          import_outcome: input.import_outcome ?? null,
        })),
        new Promise<never>((_resolve, reject) => {
          timeout = setTimeout(() => {
            agent?.abort()
            reject(new PiRuntimeError('PI_MODEL_TIMEOUT', 'Data preparation exceeded its timeout.'))
          }, this.timeoutMs)
        }),
      ])
      this.assertCurrent(generation)
      if (finalOutcome === undefined) {
        throw new PiRuntimeError('PI_DRAFT_MISSING', 'The model did not return preparation options.')
      }
      finalOutcome = {
        ...record(finalOutcome, 'Preparation result'),
        model_turn_count: completedTurns,
        tool_call_count: toolCalls,
      ...
      this.emit(
        ...
        projectId,
        'completed',
        record(finalOutcome, 'Preparation result').outcome === 'proposal'
          ? '已提出数据整理方案，正在执行本地校验。'
          : '当前证据不足，未修改数据。',
      )
      return finalOutcome
      ...
      this.emit(clientRunId, projectId, 'failed', 'Agent 未能提出安全的数据整理方案。')
      throw error
    } finally {
